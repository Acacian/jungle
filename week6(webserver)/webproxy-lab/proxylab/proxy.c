...
{
	int cache_on = 1; // 동시성 처리하려고.. 같은 메모리쓰면 안되니까
	int port;
	...
	// ignore SIGPIPE
		...
		// 연결이 종료되었을 때마다 프로세스를 종료할 수 없으니 무시하는 것. 쉽게 말해 통신이 끊어지면 에러가 나는데 그 에러를 무시하게 해 주는 함수
		// 그게 pf조건
...
void proxy_process(int *arg){
	...
	Pthread_detach(Pthread_self()); //detach : 분리 > fork 뜨는거랑 비슷한 느낌 > 대신 메인 thread가 대기를 하지 않아도 된다. 같이 흘러가니 비동기 > 메인 thread는 계속 받아줘야한다
	
	int client_fd = (int)(*arg); //connect fd라고 가정
	int server_fd = -1;
	
	char tmp_str[MAXBUF]; // 임시버퍼
	
	char request_str[MAXBUF]; // 어떤요청인가
